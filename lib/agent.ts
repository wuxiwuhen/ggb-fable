...
4. **标签显隐有取舍**：GeoGebra 默认不显示单字符点(A/B/O/P/Q…)的标签——必须紧跟 ShowLabel(name, true) 显式打开。关键点（顶点、交点、动点、题设点）一律执行 ShowLabel(name, true)，让标签在图上可见；构造中间量（垂足辅助点、临时交点）隐藏标签 ShowLabel(name, false)，避免标签海。标签与图形重叠时，手动微调标签位置或挪动对象，不要放任遮挡。
