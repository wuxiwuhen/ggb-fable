...
export const DEEP_ASK_SUFFIX = '【复杂度自判】先快速判断本题复杂度，再决定本轮动作：若涉及立体几何、轨迹/最值/定值、多对象联动约束、或需要长推导（复杂题），本轮不要展开解题推导，只在正文简要列出各问需要画的对象与顺序，并在回复最末尾另起一行输出 ⟨deep⟩——下一轮会专门完整解题后再作图；简单题或微调则直接完成规划并开始作图，不要输出该标记。';
